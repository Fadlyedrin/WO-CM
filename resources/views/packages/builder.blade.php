    ...
    <div class="builder-container">
        ...
        <div class="builder-left">
            ...
            <div class="builder-section">
                        ...
                        <div class="component-body">
                            ...
                            @if($component->description)
                                <div class="component-desc">
                                    {{ Str::limit(strip_tags($component->description), 120) }}
                                    @if(strlen(strip_tags($component->description)) > 120)
                                        <div class="mt-1">
                                            <a href="#" class="read-more-btn fw-bold text-primary text-decoration-none" style="font-size:0.8rem;" data-bs-toggle="modal" data-bs-target="#descModal{{ $component->id }}">Lihat Detail <i class="bi bi-arrow-right-short"></i></a>
                                        </div>
                                    @endif
                                </div>
                            @endif
                ...
                {{-- Modals for Component Details --}}
                @foreach($package->components as $component)
                    @if($component->description && strlen(strip_tags($component->description)) > 120)
                    <div class="modal fade" id="descModal{{ $component->id }}" tabindex="-1">
                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                            <div class="modal-content rounded-4 border-0">
                                <div class="modal-header border-0 pb-0">
                                    <h5 class="modal-title fw-bold"><i class="bi {{ $component->icon }} text-primary me-2"></i>{{ $component->name }}</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                </div>
                                <div class="modal-body p-4" style="line-height:1.7; font-size:0.95rem; color:#444;">
            ...
            <div class="price-summary-card" id="price-summary">
                ...
                <div class="summary-body">
                    ...
                    <div class="summary-components" id="summary-components">
                        <div class="summary-item" id="summary-base-fee" style="display:none;">
                            <span class="summary-item-name">
                                <i class="bi bi-building text-primary me-1"></i>
                                Biaya Dasar Paket
                            </span>
                            <span class="summary-item-price" id="base-fee-price">Rp 0</span>
                        </div>
                        @foreach($package->components as $component)
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let totalAllComponents = 0;
    document.querySelectorAll('.component-checkbox').forEach(cb => {
        totalAllComponents += parseInt(cb.dataset.price);
    });
    let baseFee = basePrice - totalAllComponents;
    ...
    function updateSummary() {
        ...
        // Sum mandatory components
        const requiredComponents = document.querySelectorAll('.component-checkbox:disabled');
        requiredComponents.forEach(cb => {
            totalCheckedPrice += parseInt(cb.dataset.price);
        });
        ...
        // Show base fee in summary if exists
        const baseFeeItem = document.getElementById('summary-base-fee');
        if (baseFee > 0 && baseFeeItem) {
            baseFeeItem.style.display = 'flex';
            document.getElementById('base-fee-price').textContent = formatRp(baseFee);
        }

        let newTotal = baseFee + totalCheckedPrice;

        // Fallback: if there are no components at all, just show the base price
        if (document.querySelectorAll('.component-checkbox').length === 0) {
            newTotal = basePrice;
        }

        totalPriceEl.textContent = formatRp(newTotal);
        
        // Hide savings badge as we don't calculate deduction
    ...
    checkboxes.forEach(cb => {
        ...
        card.addEventListener('click', function (e) {
            ...
            if (e.target.closest('.read-more-btn')) return; // Ignore click on Read More link
            if (!cb.disabled) {
